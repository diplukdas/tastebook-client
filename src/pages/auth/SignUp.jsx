...
import { setCredentials } from "../../features/auth/authSlice"; // Import action creator
...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const userDataWithRole = { ...formDetails, roles: ["ProUser"] };
      const userData = await toast.promise(
        signUp(userDataWithRole).unwrap(),
        {
          pending: "Please wait...",
  ...
  return (
    <section className="flex w-full h-screen bg-loginbg">
      {/* Sign up form container */}
      <div className="basis-1/4 m-auto flex flex-col">
        <Logo customCss={"mx-auto md:mx-0 filter invert"} />
      ...
      <div className="hidden md:block basis-1/2 bg-no-repeat bg-cover bg-center"></div>
    </section>
  );
};
