...
//                   required
//                   onBlur={handleFocus}
//                   focused={focused.category ? focused.category.toString() : ""}
//                   aria-required="true"
//                   className="p-1.5 border bg-gray-100 rounded focus:outline outline-primary"
//                 >
...
const EditRecipe = () => {
  ...
  useEffect(() => {
    if (!rest?.isLoading) {
      setFormDetails({
        ...
        ingredients: data?.ingredients,
        instructions: data?.instructions,
        category: data?.category, // Update category field
      });
    }
  }, [rest?.isLoading]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Update the recipe using the updateRecipe mutation
      ...
      {rest.isLoading ? (
        ...
        >
          <div className="basis-1/2 flex flex-col gap-5">
            ...
            {/* Add the category dropdown/select */}
            <div className="flex flex-col sm:flex-row justify-between">
              <label
                htmlFor="category"
                className="text-sm font-semibold mb-3 basis-1/2"
